...
public class AdminServiceImpl extends SuperServiceImpl<AdminMapper, Admin> implements AdminService {
    ...
    public Admin getMe() {
        ...
        Admin admin = getById(request.getAttribute(SysConf.ADMIN_UID).toString());
        //清空密码，防止泄露
        admin.setPassWord(null);
        //获取图片
        if (StringUtils.isNotEmpty(admin.getAvatar())) {
    ...
    public String getList(AdminVO adminVO) {
        ...
        IPage<Admin> pageList = page(page, queryWrapper);
        ...
        for (Admin item : list) {
            ...
            if (StringUtils.isNotEmpty(item.getAvatar())) {
                ...
                pictureUidsTemp.forEach(picture -> {
                    if (pictureMap.get(picture) != null && !"".equals(pictureMap.get(picture))) {
    ...
    public String addAdmin(AdminVO adminVO) {
        ...
        QueryWrapper<Admin> queryWrapper = new QueryWrapper<>();
        queryWrapper.eq(SQLConf.USER_NAME, userName);
        Admin temp = getOne(queryWrapper);
        if (temp == null) {
    ...
    public String editAdmin(AdminVO adminVO) {
        Admin admin = getById(adminVO.getUid());
        ...
        List<Admin> adminList = list(queryWrapper);
        if (adminList != null) {
