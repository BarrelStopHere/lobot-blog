...
import top.lobot.sms.global.RedisConf;
import top.lobot.sms.global.SysConf;
import top.lobot.utils.JsonUtils;
import top.lobot.utils.RedisUtil;
import top.lobot.utils.SpringUtils;
import top.lobot.base.enums.ESearchModel;
...
public class BlogListener {
    ...
    @RabbitListener(queues = "lobot.blog")
        ...
        if (map != null) {
            ...
            try {
                switch (comment) {
                    ...
                    break;
                    default: {
                        log.info("sms处理博客兜底方法");
                    }
                }
            } catch (Exception e) {
                e.printStackTrace();
                log.error("【sms】出现异常，请查看search是否启动！searchModel: " + searchModel);
            }
        }
    }
